...
def get_titles_by_lang(lang="ru"):
    TITLES = {
        "ru": [
            (50,  "🌱 Новичок"),
            (100, "✨ Мотиватор"),
    ...
    return TITLES.get(lang, TITLES["ru"])
    
def get_user_points(user_id: str) -> int:
    stats = load_stats()
    return int(stats.get(str(user_id), {}).get("points", 0))

def get_next_title_info(points, lang="ru"):
    TITLES = get_titles_by_lang(lang)
    for threshold, title in TITLES:
        if points < threshold:
            return title, threshold - points
    return TITLES[-1][1], 0  # Уже максимальное звание

def build_titles_ladder(lang="ru"):
    TITLES = get_titles_by_lang(lang)
    ladder_lines = []
    for threshold, title in TITLES:
        if threshold == float("inf"):
            ladder_lines.append(f"{title} — ∞ очков")
        else:
            ladder_lines.append(f"{title} — {threshold} очков")
    return "\n".join(ladder_lines)
