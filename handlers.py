...
# Список режимов
MODES = {
    "support": "Ты — добрый и поддерживающий AI-компаньон, который помогает справиться с трудными моментами. Ты очень чуткий, тёплый и спокойный.",
    "motivation": "Ты — вдохновляющий и заряжающий AI-компаньон. Помогаешь поверить в себя, мотивируешь и поддерживаешь уверенность.",
    "psychology": "Ты — внимательный, рассудительный и очень деликатный AI, похожий на хорошего психолога. Ты задаёшь вопросы и помогаешь разобраться в себе.",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"{icon} Сейчас включён режим общения: *{mode}*\n_({prompt})_", parse_mode="Markdown")
...
# Команда /mode — показать emoji-меню
async def mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("🌱 Поддержка", callback_data="mode:support")],
        [InlineKeyboardButton("✨ Мотивация", callback_data="mode:motivation")],
        [InlineKeyboardButton("🧠 Психолог", callback_data="mode:psychology")],
        [InlineKeyboardButton("🎭 Юмор", callback_data="mode:humor")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Выбери стиль общения Mindra ✨", reply_markup=reply_markup)

# Обработка выбора emoji-режима
async def mode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode in MODES:
        ...
        save_history(conversation_history)
        await query.edit_message_text(f"✅ Режим общения изменён на *{mode}*!", parse_mode="Markdown")
