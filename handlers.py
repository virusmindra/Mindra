...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/goal — поставить цель (например: `/goal Прочитать 10 страниц книги`)\n"
        "/goals — посмотреть свои цели\n"
        "/done — отметить цель выполненной (`/done 1`)\n"
        "/delete — удалить цель (`/delete 1`)\n"
        "Скоро появятся и другие фишки 😉",
        parse_mode="Markdown"
    )
