...
def detect_topic_and_react(user_input: str) -> str:
    text = user_input.lower()
    # Тема: любовь / отношения
    elif re.search(r"\b(девушк|люблю|влюбил|отношен|парн|расстался|развод|свидани|флирт|поцелуй)\b", text):
        return "💘 Это звучит очень важно. Отношения могут быть сложными, но ты не один в этом. Расскажи, что ты чувствуешь сейчас?"

    # Тема: одиночество
    elif re.search(r"\b(одинок|один|некому|никто не)\b", text):
        return "🫂 Иногда нам не хватает тепла и понимания... Но знай: ты не один. Я здесь, чтобы поддержать тебя."

    # Тема: работа / стресс
    elif re.search(r"\b(работа|устал|босс|давлени|коллег|увольн|смена|заработ|не выношу)\b", text):
        return "🧑‍💼 Работа и давление могут действительно выматывать. Ты уже многое выдержал. Давай поговорим об этом подробнее?"

    # Тема: спорт / успех
    elif re.search(r"\b(зал|тренир|спорт|бег|жим|пресс|100кг|гантел|похуд|пресс|добился)\b", text):
        return "🏆 Какой молодец! Я горжусь твоим прогрессом. Поделись, что именно у тебя получилось — это важно!"

    # Тема: семья
    elif re.search(r"\b(мама|папа|родител|семь|брат|сестра|дед|бабушк|родня)\b", text):
        return "👨‍👩‍👧‍👦 Семья — это опора, но и сложная часть жизни. Если что-то тревожит — расскажи, я рядом."

    # Тема: саморазвитие, мотивация
    elif re.search(r"\b(мотивац|саморазвити|цель|достиг|развитие|путь|дух|медитац|осознанн|мудро|успех)\b", text):
        return "🌱 Здорово, что ты движешься вперёд. Такой путь требует усилий, и ты их уже вкладываешь. Давай разберёмся вместе?"

    # Если тема не определена — ничего не добавляем
    return ""
