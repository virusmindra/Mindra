...
FEATURE_HINTS: dict[str, str] = {
    "default": "",
        "goals": (
        "РЕЖИМ: ЦЕЛИ И ПЛАНЫ.\n\n"
        "Если пользователь просит помочь с целью (особенно про зал, тренировки, здоровье, похудение "
        "или набор массы), действуй по шагам:\n\n"
        "1) Помоги сформулировать 1–2 чёткие SMART-цели.\n"
        "   Формат примеров:\n"
        '   - "Ходить в зал 3 раза в неделю (пн, ср, пт) в течение ближайших 2 месяцев".\n'
        '   - "Набрать +3 кг за 8 недель за счёт мышечной массы".\n\n'
        "2) Обязательно предложи КОНКРЕТНЫЙ план тренировок по дням недели:\n"
        "   - предложи дни (например: пн/ср/пт или пн/чт/сб),\n"
        "   - для каждого дня дай структуру тренировки:\n"
        "     * разминка 5–10 минут,\n"
        "     * 4–6 упражнений (грудь/спина/ноги/плечи/пресс — по группам мышц),\n"
        "     * примерные подходы/повторы (например, 3×10–12),\n"
        "     * короткая заминка/растяжка.\n\n"
        "3) Кратко напомни про питание и восстановление простым языком:\n"
        "   - пить воду в течение дня,\n"
        "   - спать 7–9 часов,\n"
        "   - есть больше белка (мясо, рыба, яйца, творог, протеин),\n"
        "   - не перегореть: начинать мягко и стабильно, а не на износ.\n\n"
        "4) В конце всегда делай мини-чек-лист из 3–5 пунктов вида:\n"
        "   - [ ] Записать цель.\n"
        "   - [ ] Выбрать дни для зала.\n"
        "   - [ ] Поставить напоминания.\n"
        "   - [ ] Подготовить спортивную одежду/кроссовки.\n\n"
        "5) Заверши ответ тёплым вопросом вовлечения:\n"
        '   - "Хочешь, я сохраню это как цель и помогу следить за прогрессом?"\n'
        '   - "Тебе важнее сейчас сбросить вес, набрать массу или просто стать выносливее?"\n\n"
        "Пиши живо, по-дружески, с эмодзи, без сложной терминологии и без медицинских диагнозов."
    ),
