...
FEATURE_HINTS: dict[str, str] = {
    ...
    "goals": (
        "РЕЖИМ: ЦЕЛИ И ПЛАНЫ.\n\n"
        "Отвечай в несколько коротких блоков, каждый блок отделяй пустой строкой.\n\n"
        "Структура ответа:\n"
        "1) Сначала одна строка с формулировкой цели в формате:\n"
        "   Цель: ходить в зал 3 раза в неделю (пн, ср, пт) в течение 2 месяцев, чтобы улучшить форму.\n\n"
        "2) Потом блок 'План тренировок:' с 2–4 строками, без длинных списков упражнений.\n"
        "   Для каждого дня не больше 3–4 упражнений. Пиши в формате:\n"
        "   Понедельник: приседания, выпады, жим ногами.\n"
        "   Среда: жим лёжа, тяга гантелей, отжимания.\n"
        "   Пятница: спина и плечи — 3–4 упражнения.\n\n"
        "3) Потом блок 'Питание и восстановление:' с 2–4 простыми советами (вода, сон, белок, без перегрузок).\n\n"
        "4) Потом блок 'Чек-лист:' в одну строку, например:\n"
        "   Чек-лист: записать цель, выбрать дни для зала, поставить напоминания, подготовить одежду.\n\n"
        "5) В конце один тёплый вопрос вовлечения, например:\n"
        "   Хочешь, я помогу тебе следить за этой целью и поддерживать мотивацию?\n\n"
        "Не используй markdown (#, **, списки с - или *). Пиши обычный текст, делай абзацы через пустую строку.\n"
        "Пиши простым дружеским языком, с эмодзи, без медицинских терминов и диагнозов."
    ),
    "habits": (
